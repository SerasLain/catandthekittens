...
"""Конвертирует pdf в текст и удаляет всё ненужное. Не удаляются примечания внизу страницы. Не сохраняется деление
на абзацы.
Считает суммарное количество слов в текстовых файлах из одной директории."""
...
if __name__ == '__main__':
    ...
    for i,file in enumerate(os.listdir('articles2015')):
        with open("cleaned_issues_in_linguistics_2015\cleaned_iil_2015{0}.txt".format(i),'w',encoding='utf-8') as f:
            text,count = document_to_text('articles2015/{0}'.format(file))
            print(count)
            f.write(text)
            total_count+=count
